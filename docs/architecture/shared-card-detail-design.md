...
- Een kleine pure projector bepaalt uit rows of een kaart `absent`, `manageable`, `present-unmanageable` of `conflict` is. Dezelfde projector wordt door batch- en single-cardreads gebruikt.
...
type ConfirmedOwnership =
  | { kind: 'absent' }
  | { kind: 'manageable'; record: OwnershipRecord }
  | { kind: 'present-unmanageable'; records: OwnershipRecord[] }
  | { kind: 'conflict'; records: OwnershipRecord[]; reason: string };
...
IDs en positieve quantity zijn verplicht gevalideerd. Onbekende statuswaarden worden niet stil als afwezig of beheerbaar behandeld: de service levert een veilig conflict/ongeldig-resultaat. `present-unmanageable` is normale, bevestigde data (bijvoorbeeld wishlist, trade, missing of een andere conditie); `conflict` betekent ambigue of invalide data. Alleen `ready/absent` bewijst afwezigheid.
...
| Bevestigd afwezig | Toon “Niet in collectie”; bied add alleen aan als capability en contract dit toestaan. |
...
| Meerdere/niet-beheerbare rows | Toon bevestigde aanwezigheid met “Beheer via collectie”, of “Gegevensconflict” bij ambiguïteit; voer geen quantitymutatie uit. |
...
- **Wishlist:** dezelfde kaart en ownershipunion; adapter bepaalt statusgerichte copy en toegestane overgang, niet de presentational component.
- **Trade:** meerdere records/condities blijven expliciet; toekomstige trade-acties worden nieuwe capabilities/intenties en geen conditionals op een paginanaam.
- **Search:** catalogusresultaten leveren hetzelfde `CardDetailCard`; ownership wordt begrensd voor alleen zichtbare/gekozen IDs geladen, nooit via een volledige-catalogusread.

Nieuwe statussen of acties breiden het domeincontract en de serviceprojector bewust uit. De basiscomponent wordt niet vooraf een algemene workflow-engine en bevat geen Wishlist-, Trade- of Search-querylogica.

## Kleine implementatiefasen

1. **Contracten en servicegrens:** introduceer gedeelde typen en generieke services buiten `setsPage`; behoud bestaande exports tijdelijk als compatibiliteitslaag en bewijs met tests dat reads, add, ±1, delete en conflictsemantiek gelijk blijven.
2. **Shared presentational detail + Sets-adapter:** extraheer de huidige Sets-detailflow met functionele en mobiele visuele parity; voeg focus trap, inert achtergrond en ownership-retry gecontroleerd toe. Binder, paginatie en setvoortgang blijven verder ongewijzigd.

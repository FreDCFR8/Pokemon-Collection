-- Phase 7B set-catalog recovery: read-only postcheck and idempotency check
-- Dataset: PokemonTCG/pokemon-tcg-data@0af6250a22495e4a3e9f60ff45fc3fedc2e0563d
-- Expected result after successful recovery:
-- exactSets = 117, exactReferences = 117, existingSetCodeConflicts = 117,
-- existingSourceIdentityConflicts = 117, existingReferenceConflicts = 117,
-- remainingWriteDelta = 0.
--
-- This file contains SELECT statements only. It does not create, update, or delete data.

with expected (set_code, name, series, source, external_id) as (
  values
...
reference_conflicts as (
  select count(*)::integer as count
  from expected
  join public.set_external_references reference
    on reference.source = expected.source
   and reference.external_id = expected.external_id
)
select jsonb_build_object(
  'expectedSets', 117,
  'exactSets', (select count from exact_sets),
  'expectedReferences', 117,
  'exactReferences', (select count from exact_references),
  'existingSetCodeConflicts', (select count from set_code_conflicts),
  'existingSourceIdentityConflicts', (select count from source_identity_conflicts),
  'existingReferenceConflicts', (select count from reference_conflicts),
  'remainingWriteDelta', case
    when (select count from exact_sets) = 117
     and (select count from exact_references) = 117
     and (select count from set_code_conflicts) = 117
     and (select count from source_identity_conflicts) = 117
     and (select count from reference_conflicts) = 117
    then 0
    else null
  end
) as phase_7b_postcheck;

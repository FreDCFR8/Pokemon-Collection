...
async function validateOne(supabase: SupabaseClient, manifestSet: LocalCatalogManifestSet, candidate: CandidateRecord, inputRoot: string, preflightReasonCodes: SetMappingReasonCode[] = []): Promise<ValidationRecord> {
  ...
  const linkedCards = await rows<CatalogCard>(supabase.from('cards_catalog').select('id,set_code,number').in('id', linkedIds), 'cards_catalog referenced cards');
  ...
  for (const [externalId, refs] of [...refsByExternalId.entries()].sort()) {
    ...
    if (refs.length > 1) { conflicts += 1; cardReferenceExamples.push({ external_id: externalId, card_catalog_id: refs[0].card_catalog_id, expected_number: expected.number, reason: 'multiple_card_references' }); continue; }
...
async function main(): Promise<number> { try { const options = parseArgs(process.argv.slice(2)); const manifest = parseLocalCatalogManifestFromText(readFileSync(options.manifestPath, 'utf8')); validateLocalDatasetCheckout(options.inputRoot, manifest.datasetVersion); const url = process.env.SUPABASE_URL; const key = process.env.SUPABASE_SERVICE_ROLE_KEY; if (!url || !key) throw new Error('SUPABASE_URL en SUPABASE_SERVICE_ROLE_KEY zijn vereist voor read-only databasevalidatie.'); const report = await runValidation({ ...options, supabase: createClient(url, key) }); writeAtomicJson(options.reportPath, report); summarize(report); return 0; } catch (error) { console.error(`Setmappingvalidatie geblokkeerd: ${error instanceof Error ? error.message : 'onbekende fout'}`); return 1; } }
